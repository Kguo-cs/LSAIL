...
class LQR_solver():
    def __init__(self,n_state,n_ctrl,T,lqr_iter=10,max_linesearch_iter=10,u_lower=None,u_upper=None):
        ...
        for t in range(self.T-1, -1, -1):
            ...
            if self.u_lower is None:
                ...
                kt, Qt_uu_free_LU, If, n_qp_iter = pnqp(
                    Qt_uu, qt_u, lb, ub,
                    x_init=prev_kt, n_iter=self.pnqp_iter)
    ...
    def solve(self, x_init,C,c,F):
        ...
        # acc=torch.linalg.norm(u[0],dim=-1)
        #
        # print(torch.mean((acc>3).to(float)))
